...
export default function ContasPage() {
  ...
  const { modalConta, conta: modalContaActions } = useModais()
  ...
  const recarregarContas = async () => {
    ...
    try {
      setCarregando(true)
      setErro(null)
      const dadosContas = await obterContasComSaldo(workspace.id)
  ...
  const handleExcluir = async (id: string) => {
    if (confirm('Tem certeza que deseja excluir permanentemente esta conta?')) {
      try {
        ...
        await excluirConta(id, workspace.id)
  ...
  return (
      <div className="max-w-7xl mx-auto space-y-6">
        <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
          ...
          <Button onClick={() => modalContaActions.abrir()}>
                      ...
                      <TableCell>
                        <div className="flex gap-1 justify-center">
                          <Button
                            variant="ghost"
                            size="sm"
                            onClick={() => modalContaActions.abrir(conta.id)}
                            title="Editar conta"
        ...
        {/* Modal de Conta */}
        <ModalConta
          isOpen={modalConta.isOpen}
          onClose={modalContaActions.fechar}
          onSuccess={recarregarContas}
          contaId={modalConta.entidadeId}
        />
      </div>
  )
}